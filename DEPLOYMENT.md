...
- `api/communicate.js` is the server-side GPT proxy. Deploy it to a Node-compatible serverless platform and store `OPENAI_API_KEY` only in that platform's secret environment settings.
